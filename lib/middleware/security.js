...
function configureCORS(req, res, next) {
  ...
  const allowedOrigins = process.env.ALLOWED_ORIGINS 
    ...
    : [
        ...
        'https://fireberry-form-registration-git-securi-5edec4-ameidars-projects.vercel.app',
...
/**
 * Combined security middleware (simplified for debugging)
 * @param {object} req - Request object
 * @param {object} res - Response object
 * @param {function} next - Next function
 */
function applySecurity(req, res, next) {
  ...
  configureCORS(req, res, (err) => {
    ...
    // For now, just do basic logging and skip advanced security features
    try {
      logRequest(req, res, (err) => {
        if (err) {
          console.error('Logging error:', err);
          return res.status(500).json({ error: 'Logging error' });
        }
        next();
      });
    } catch (error) {
